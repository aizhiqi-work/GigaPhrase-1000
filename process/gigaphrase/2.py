...
def process_example(example):
    timestamps = example.get('words', '[]')  # 防止 None 或空值导致出错
    ...
    for start_idx in range(len(timestamps)):
        for window_len in range(1, window_size + 1):  # 窗口长度为1到4
            ...
            if end_idx < len(timestamps):
                ...
                # 时间和长度过滤
                if 0.5 <= duration <= 2.0 and concat_len >= min_len and min_score >= 0.5:
...
if __name__ == '__main__':
    ...
    result_dataset = dataset.map(
        process_example,
        num_proc=32,                    # 多进程，可调整为你的 CPU 核心数
    ...
    print("\n💾 正在保存结果...")
    for n in range(1, 5):
        filename = f"/nvme01/openkws/libriphrase/counts/gs-1000/ngram_{n}.txt"
