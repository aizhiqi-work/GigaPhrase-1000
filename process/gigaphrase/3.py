    ...
    for start_idx in range(len(timestamps)):
        for window_len in range(1, window_size + 1):
            ...
            if end_idx < len(timestamps):
                # 获取时间窗口
                window_start = timestamps[start_idx]["start_time"]
                window_end = timestamps[end_idx]["end_time"]
                duration = window_end - window_start
                scores = [
                    timestamps[i]["score"] for i in range(start_idx, end_idx + 1)
                ]
                min_score = min(scores)
                if 0.5 <= duration <= 2.0 and min_score >= 0.5:
...
if __name__ == "__main__":
    parquet_file = "/nvme01/openkws/libriphrase/counts/gs-1000/list.parquet"
    dataset_df = pd.read_parquet(parquet_file)
    dataset = Dataset.from_pandas(dataset_df)

    # 加载全局高频 n-gram 集合
    ngram_sets = load_ngram_dict("/nvme01/openkws/libriphrase/counts/gs-1000", min_count=10)
    ...
    out_file = "/nvme01/openkws/libriphrase/counts/gs-1000/matched_sentences.parquet"
